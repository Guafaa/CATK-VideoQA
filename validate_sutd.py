...
def validate(cfg, model, data, device, write_preds=False):
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            ...
            loss,logits = model(answers,*batch_input, video_ids)
            ...
            # 收集本批错误样例（最多收集到 error_examples 中，稍后统一打印）
            try:
                ans_vocab = data.vocab['answer_idx_to_token']
                ques_vocab = data.vocab['question_idx_to_token']
                B = answers.size(0)
                T = batch_input[-1].size(1)
                for i in range(B):
                    if not bool(agreeings[i].item()):
                        # 重建问题文本
                        tokens = []
                        for j in range(T):
                            idx = int(batch_input[-1][i, j].to('cpu'))
                            if idx != 0:
                                tokens.append(ques_vocab[idx])
                        question_text = ' '.join(tokens)
                        # 取视频/问题ID
                        vid = int(video_ids[i].item()) if hasattr(video_ids[i], 'item') else video_ids[i]
                        qid = int(question_ids[i].item()) if hasattr(question_ids[i], 'item') else question_ids[i]
                        # 预测与真值
                        pred_ans = ans_vocab[int(preds[i].item())]
                        gt_ans = ans_vocab[int(answers[i].item())]
            ...
            if cfg.dataset.name == 'sutd-qa':
                ...
                reverse_idx = []
                # 打印前几个问题的token内容
                for i in range(min(3, batch_input[-1].size(0))):  # 只打印前3个问题
                    question_tokens = []
                    for j in range(batch_input[-1].size(1)):
                        token_idx = int(batch_input[-1][i, j].to('cpu'))
                        if token_idx != 0:  # 跳过padding
                            token = data.vocab['question_idx_to_token'][token_idx]  # 这里用int
                            question_tokens.append(token)
                    # print(f"问题 {i}: {' '.join(question_tokens)}")
        ...
        if cfg.dataset.name == 'sutd-qa':
            ...
            print(f'basic_count={basic_count}, attri_count={attri_count}, intro_count={intro_count}, counter_count={counter_count}, forcast_count={forcast_count}, reverse_count={reverse_count}')
            basic_acc = basic_acc / basic_count if basic_count != 0 else 0
            attri_acc = attri_acc / attri_count if attri_count != 0 else 0
            intro_acc = intro_acc / intro_count if intro_count != 0 else 0
            counter_acc = counter_acc / counter_count if counter_count != 0 else 0
            forcast_acc = forcast_acc / forcast_count if forcast_count != 0 else 0
            reverse_acc = reverse_acc / reverse_count if reverse_count != 0 else 0

    # 打印若干错误样例
    if len(error_examples) > 0:
        print('Display up to 5 error examples...')
        for ex in error_examples[:5]:
            print(f"[video_id={ex['video_id']} | question_id={ex['question_id']}]")
            print(f"Q: {ex['question']} ?")
            print(f"Pred: {ex['pred']} | GT: {ex['gt']}")
